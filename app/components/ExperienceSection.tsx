...
// Non-translatable static data per timeline entry (by index)
...
type TimelineEntry = TimelineMsg & {
  muted?:   boolean;
  current?: boolean;
  proof?:   string;
};
...
export default function ExperienceSection() {
  ...
  const TIMELINE: TimelineEntry[] = (t.raw("timeline") as TimelineMsg[]).map((msg, i) => ({
    ...msg,
    ...TIMELINE_STATIC[i],
  }));
            ...
            {TIMELINE.map((entry, i) => (
              ...
              >
                ...
                <div className="relative z-10 shrink-0 mt-[3px]">
                  <div className={`w-[15px] h-[15px] rounded-full border bg-[#0d0f16] flex items-center justify-center ${entry.muted ? "border-gray-600/40" : "border-teal-500/40"}`}>
                    <div className={`w-[7px] h-[7px] rounded-full ${entry.muted ? "bg-gray-500" : "bg-teal-400 shadow-[0_0_6px_rgba(20,184,166,0.8)]"}`} />
                  </div>
                </div>
                ...
                <motion.div
                  ...
                  className={`flex-1 border rounded-2xl p-6 transition-colors duration-200 ${entry.muted ? "bg-[#0f1017] border-white/5 hover:border-white/10" : "bg-[#111218] border-white/8 hover:border-teal-500/20"}`}
                >
                  {/* Period + role */}
                  <div className="flex flex-wrap items-start justify-between gap-2 mb-1">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className={`text-[11px] font-bold font-mono px-2.5 py-1 rounded-full border ${entry.muted ? "bg-gray-500/10 border-gray-500/20 text-gray-500" : "bg-teal-500/10 border-teal-500/22 text-teal-400"}`}>
                        {entry.period}
                      </span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-2 mb-5">
                    {entry.bullets.map((b, bi) => (
                      <li key={bi} className="flex items-start gap-2.5 text-sm text-gray-400 leading-relaxed">
                        <span className={`mt-[5px] shrink-0 w-1.5 h-1.5 rounded-full ${entry.muted ? "bg-gray-600" : "bg-teal-500/60"}`} />
                        {b}
                      </li>
                    ))}
                  </ul>
                  ...
                  {entry.proof && (
                    <div className="mt-2">
                      ...
                      <div className="group/proof relative w-full rounded-xl overflow-hidden border border-white/10 bg-[#1a1d27]">
                        <Image
                          src={entry.proof}
                          alt={entry.caption ?? t("trafficProofLabel")}
                          width={1200}
                          height={600}
                          quality={100}
                          unoptimized
