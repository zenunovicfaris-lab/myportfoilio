...
function CircularButton({ faIcon, imgSrc, onClick, variant = "circular-button-variant-default", size = "circular-button-size-default", tooltip = null, className = "" }) {
    return (
        <button className={`circular-button ${variant} ${size} ${className}`}
                data-tooltip={tooltip}
                onClick={onClick}>
            {imgSrc ? (
                <img src={imgSrc} alt="" className="circular-button-img" />
            ) : (
                <i className={`${faIcon}`}/>
            )}
        </button>
...
CircularButton.Variants = {
    ...
    DARK: "circular-button-variant-dark",
}
